...
function LinesQuery() {
    // --- State variables --- (Keep all state variables as defined previously)
    ...
    // --- useEffect and Helper functions --- (Keep as defined previously)
    useEffect(() => {
    ...
    const handleGenerateCombinations = async () => {
        ...
        try {
            const response = await axios.post(`${API_BASE_URL}/linesPER/`, { /* top_n: 50 */ }); // POST for generation
            if (response.data && Array.isArray(response.data.top_lineups)) {
                 setOptimalLineups(response.data.top_lineups);
                 if (response.data.top_lineups.length === 0) { setGenerationError("No valid lineups could be generated."); }
            } else { setGenerationError("Received unexpected data structure."); }
        } catch (err) { console.error("Generate Lineups API error:", err); let errorMsg = err.message; if (err.response?.data?.error) { errorMsg = err.response.data.error; } setGenerationError(errorMsg); setOptimalLineups([]); }
        finally { setIsGeneratingLines(false); }
    };
    ...
    return (
        <Container>
            ...
            <Row className="mt-3">
             ...
             {/* Player PER Section - Renders if initial load OK */}
             {/*
            ...
            {!isGeneratingLines && !generationError && optimalLineups.length > 0 && (
                 <Row className="mt-4">
                    <Col>
                        <h4>Top Lineups by Total Player PER</h4>
